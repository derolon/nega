...
class Title:
    ...
    def update_title(self):
            try:
                ...
                title = f'999 Shops Gen | G: {genned} | E: {errors} | S: {solved} | U: {unlocked} | L: {locked} | Unlock Rate: {unlock_rate}% | Elapsed: {round(time() - genStartTime, 2)}s |'
...
class Cookie:
    def __init__(self, proxy, session):
        self.user_agent = useragent # Default useragent
        self.proxy = proxy
    ...
    def get_cookies(self):
        while True:
            try:
                ...
                try:
                    Log.Error(f"Failed to get cookies: {e} | Proxy: {self.proxy[:20]}.....")
                except:
...
if __name__ == "__main__":
    ...
    title.update_title()
    os.system('cls' if os.name == 'nt' else 'clear')
    threads = int(input(Fore.LIGHTBLUE_EX + "Threadcount: " + Fore.RESET))
